...
def list_assets(status: str | None = None, location_id: str | int | None = None):
    ...
    try:
        ...
        if location_id:  filters.append("location_id = ?");  params.append(location_id)
...
def create_asset(
    name: str,
    category: str,
    purchase_date=None, cost=None, status="available",
    serial_no=None, notes=None, location_id=None, assigned_to=None
):
    conn = get_conn(); cur = conn.cursor()
    try:
        # Handle optional assigned_to gracefully if the column exists
        has_assigned = _has_column(cur, 'assets', 'assigned_to')
        if has_assigned:
            cur.execute("""
                INSERT INTO assets (name, category, purchase_date, cost, status, serial_no, notes, location_id, assigned_to)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (name, category, purchase_date, cost, status, serial_no, notes, location_id, assigned_to))
        else:
            cur.execute("""
                INSERT INTO assets (name, category, purchase_date, cost, status, serial_no, notes, location_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
def update_asset(asset_id: int, data: dict) -> bool:
    ...
    conn = get_conn(); cur = conn.cursor()
    try:
        # Only allow known fields; include assigned_to if present in DB and in payload
